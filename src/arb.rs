...
pub fn get_mints_of_interest() -> Vec<Pubkey> {
    let tokens_file = std::env::var("TOKENS_JSON_PATH")
        .unwrap_or_else(|_| "tokens.json".to_string());
    println!("Using tokens file: {}", tokens_file);

    let file_content =
        fs::read_to_string(&tokens_file).unwrap_or_else(|err| {
            panic!("Unable to read tokens file {}: {}", tokens_file, err)
        });

    let token_strings: Vec<String> = serde_json::from_str(&file_content)
        .unwrap_or_else(|err| {
            panic!("Failed to parse tokens file {}: {}", tokens_file, err)
        });

    token_strings
        .iter()
        .map(|p| Pubkey::from_str(p).expect("Invalid pubkey in tokens file"))
        .collect()
}
